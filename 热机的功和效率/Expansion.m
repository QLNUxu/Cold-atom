function [ Output ] = Expansion( Control, Timeslots, phys )
...
for i = 1:eigval_States
    ...
    Ef   = Ef   + Proba(i,1)*(sum(conj(psi(:,i)).*ifft(K'.*fft(psi(:,i)))).*dx             + sum(conj(psi(:,i)).*(m*w_f^2*X.^2/2)'.*psi(:,i)).*dx             + sum(gN*abs(psi(:,i)).^4).*dx);
